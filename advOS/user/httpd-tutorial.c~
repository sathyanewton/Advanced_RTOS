...
//sends entire file fd to the request, req
static int
send_data(struct http_request *req, int fd)
{
	...
	int nread = 0;
	/*
		read is like linux systemcall, takes in file descriptor, buffer and length (read was written in lab 5 I think)
		while it returns a positive value, we write it to the output. read() returns 0 at EOF
		req->sock is the file discriptor that writes to the client
	*/
	while ((nread = read(fd, buf, sizeof(buf))) > 0) 
	{
		if (write(req->sock, buf, nread) != nread) 
		{
			die("Failed to send bytes to client");
		}
}
...
send_file(struct http_request *req)
{
	...
	// code for lab 6
	if ((fd = open(req->url, O_RDONLY)) < 0) /*req->url contains the file name requested by client (browser)*/
	{
		send_error(req, 404);
		goto end;
	}
	...
	struct Stat s;
	if ((r = stat(req->url, &s)) < 0) /*reads file status. Check for error */
	{
		send_error(req, 404);
		goto end;
	}

	if (s.st_isdir) /*not a file, its a directory!*/
	{
		send_error(req, 404);
		goto end;
	}
	
	file_size = s.st_size; /*get the file's size*/
	
	// rest of code to do work of sending the webpage ( this code was not written by us, it was given is JOS)
