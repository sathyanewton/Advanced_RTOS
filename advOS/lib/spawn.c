...
copy_shared_pages(envid_t child)
{
	// code for lab 5 -M.G
	int page_num, i;
	for (i = page_num = 0; page_num < PGNUM(UXSTACKTOP-PGSIZE); ++page_num)
    {
		if (!(uvpd[page_num>>(PDXSHIFT-PTXSHIFT)] & PTE_P) || !(uvpt[page_num] & PTE_P))
		{
        	continue;	
        }
		if (uvpt[page_num] & PTE_SHARE)
        {
			i = sys_page_map(0, (void*)(page_num<<PGSHIFT), child, (void*)(page_num<<PGSHIFT), uvpt[page_num] & PTE_SYSCALL);
        }		
        if (i > 0) 
        {
            return i;
