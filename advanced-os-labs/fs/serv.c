...
serve_read(envid_t envid, union Fsipc *ipc)
{
    ...
    if ((return_value = openfile_lookup(envid, req->req_fileid, &o)) < 0) 
    {
        return return_value;
    }

    num_bytes = file_read(o->o_file, (void *) ret->ret_buf, MIN(req->req_n, PGSIZE), o->o_fd->fd_offset);
    o->o_fd->fd_offset += num_bytes;
    return num_bytes;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
    ...
    num_bytes = file_write(o->o_file, (void *) req->req_buf, MIN(req->req_n, PGSIZE - (sizeof(int) + sizeof(size_t))), o->o_fd->fd_offset);
    o->o_fd->fd_offset += num_bytes;
    return num_bytes;
}
